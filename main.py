...
def main():
    #target_feature_map = get_dnn_features_by_imageid_and_layer(['11978233.018962'])
    #np.save('tmp', target_feature_map['11978233.018962'])
    #reshaped_target = (target_feature_map['11978233.018962']['conv5_4']).reshape((1, 14, 14, 512))
    with open('tmp.pkl', 'rb') as f:
        tmp = pickle.load(f)
        reshaped_target = tmp['conv1_2'].reshape([1]+list(tmp['conv1_2'].shape))
    target = tf.placeholder(tf.float32, reshaped_target.shape)


    inputs = tf.Variable(tf.random_normal((1, 224, 224, 3)), name='recons_image')

    tf.summary.image('recons-image', inputs)
    vgg19 = VGG19()
